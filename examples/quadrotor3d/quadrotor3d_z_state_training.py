"""
四旋翼 Z 轴位置控制器训练文件
参考 quadrotor3d_x_state_training.py 结构
注意：Z轴需要抵消重力，平衡点控制输入为 g
"""
import os
import hydra
import logging
...
# 导入Z轴位置动力学
from quadrotor3d_dynamic.pos import PositionZDynamics
...
def approximate_controller(
    ...
    这是生成预训练权重的核心函数
    注意：Z轴的平衡点控制输入为 g（悬停需要抵消重力）
    """
    # 使用 pos.py 中已有的 lqr_control 方法
    ...
    logger.info(f"平衡点控制输入 u*: {dynamics.u_equilibrium.numpy()} (应为 g={dynamics.gravity})")
...
def verify_approximation(
    ...
    logger.info("预训练验证")
    logger.info("=" * 60)
    logger.info(f"注意：Z轴平衡点控制输入为 g={dynamics.gravity:.2f}")
...
@hydra.main(
    ...
    config_name="quadrotor_pos_z_state_training",
    version_base=None,
)
def main(cfg: DictConfig):
    ...
    logger.info("=" * 60)
    logger.info("四旋翼 Z 轴位置控制器训练")
    logger.info("=" * 60)
    
    # ========== 创建动力学系统 ==========
    dt = cfg.model.dt
    position_continuous = PositionZDynamics(
        mass=cfg.model.mass,
        gravity=cfg.model.gravity,
    )
    
    logger.info(f"Z轴动力学: z_ddot = Uz - g")
    logger.info(f"重力加速度 g = {position_continuous.gravity}")
    logger.info(f"平衡点控制输入 u* = {position_continuous.u_equilibrium.item():.4f}")
    ...
    for n, limit_scale in enumerate(cfg.model.limit_scale):
        ...
        max_iter = (
            cfg.train.max_iter[n]
            if OmegaConf.is_list(cfg.train.max_iter)
            else cfg.train.max_iter
        )
        l1_reg = (
            cfg.loss.l1_reg[n]
            if OmegaConf.is_list(cfg.loss.l1_reg)
            else cfg.loss.l1_reg
        )
        candidate_weight = (
            cfg.loss.candidate_roa_states_weight[n]
            if OmegaConf.is_list(cfg.loss.candidate_roa_states_weight)
            else cfg.loss.candidate_roa_states_weight
        )
        ...
        train_utils.train_lyapunov_with_buffer(
            ...
            candidate_roa_states_weight=candidate_weight,
    ...
    if cfg.model.V_decrease_within_roa:
        ...
        plt.savefig(os.path.join(os.getcwd(), "V_roa_z.png"))
        plt.close(fig)
        logger.info("ROA 可视化已保存到: V_roa_z.png")
    ...
    logger.info(f"最优控制 U_z: {u_best.item():.6f} (应接近 g={position_continuous.gravity})")
    ...
    torch.save({
        ...
        'V_min': V_min.cpu(),
        'axis': 'z',
        'gravity': position_continuous.gravity,
    }, os.path.join(final_dir, 'u_best_z.pt'))
