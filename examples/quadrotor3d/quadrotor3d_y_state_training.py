...
# 导入Y轴位置动力学
from quadrotor3d_dynamic.pos import PositionYDynamics
...
@hydra.main(
    ...
    config_name="quadrotor_pos_y_state_training",
    version_base=None,
)
def main(cfg: DictConfig):
    ...
    logger.info("=" * 60)
    logger.info("四旋翼 Y 轴位置控制器训练")
    logger.info("=" * 60)
    
    # ========== 创建动力学系统 ==========
    dt = cfg.model.dt
    position_continuous = PositionYDynamics(
        mass=cfg.model.mass,
    ...
    if cfg.model.V_decrease_within_roa:
        ...
        plt.savefig(os.path.join(os.getcwd(), "V_roa_y.png"))
        plt.close(fig)
        logger.info("ROA 可视化已保存到: V_roa_y.png")
    ...
    logger.info(f"最优控制 U_y: {u_best.item():.6f}")
    ...
    torch.save({
        ...
        'V_min': V_min.cpu(),
        'axis': 'y',
    }, os.path.join(final_dir, 'u_best_y.pt'))
